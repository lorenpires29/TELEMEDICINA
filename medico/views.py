...
def finalizar_consulta(request, id_consulta):
    if not is_medico(request.user):  #Somente um médico pode finalizar a consulta
        add_message(request, constants.WARNING, 'Somente médicos podem acessar essa página.')
        return redirect('/usuarios/sair')
    
    consulta = Consulta.objects.get(id=id_consulta)
    # Criar validação onde o médico poderá excluir somente suas consultas
    if request.user != consulta.data_aberta.user:
        add_message(request, constants.ERROR, 'A consulta não pode ser finalizada, pois encontra-se agendada para outro Médico!')
